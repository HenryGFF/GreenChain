...
int main() {
    ...
    if (!res.componenteCadastro) return 1;

    historyBlock* primeiroHist = res.blockchain;
    componenteBlock* comp = res.componenteCadastro;

    char* hash = gerarHash(primeiroHist, comp);
    ...
    historyBlock* atual = res.blockchain;
    while (atual) {
        historyBlock* tmp = atual;
        atual = atual->next;
        free(tmp);
    }
    free(comp);
