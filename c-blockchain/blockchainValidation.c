...
char* verificarHash(historyBlock* blockchain, historyBlock* blocoAtual, componenteBlock* componente) {
    if (!blockchain || !componente) return NULL;

    unsigned int soma = somaAsciiUniversal(blockchain, blocoAtual, componente);
    unsigned int bin = manipularBinario(soma, componente);
    return gerarHexa(bin);
}

const char* verificarBlockchain(historyBlock* blockchain, componenteBlock* componente) {
    if (!blockchain || !componente) return "integridade corrompida";

    historyBlock *blocoAtual = blockchain;
    historyBlock *blocoAnterior = NULL;

    while (blocoAtual) {
        // primeiro bloco
        if (!blocoAnterior) {
            if (strlen(blocoAtual->hash_anterior) > 0 &&
                strcmp(blocoAtual->hash_anterior, "null") != 0 &&
                strcmp(blocoAtual->hash_anterior, "NULL") != 0)
                return "integridade corrompida (hash inicial inválido)";
        } else {
            if (strcmp(blocoAtual->hash_anterior, blocoAnterior->hash) != 0)
                return "integridade corrompida (hash_anterior incorreto)";
        }
        ...
        char* hashCalc = verificarHash(blockchain, blocoAtual, componente);
        if (!hashCalc) return "integridade corrompida (erro hash)";
        if (strcmp(blocoAtual->hash, hashCalc) != 0) {
            printf("hash teste: %s\n", hashCalc);
            printf("hash original:%s\n", blocoAtual->hash);
            free(hashCalc);
            return "integridade corrompida (hash divergente)";
        }
        free(hashCalc);
    ...
    return "integridade autenticada";
}


static void free_results(Results* r) {
    if (!r) return;
    historyBlock* h = r->blockchain;
    while (h) {
        historyBlock* tmp = h->next;
        free(h);
        h = tmp;
    }
    free(r->componenteCadastro);
}
...
int main(void) {
    ...
    while ((nread = fread(tmp, 1, sizeof(tmp), in)) > 0) {
        ...
        if (!buffer) { fprintf(stderr, "malloc falhou\n"); return 1; }
        memcpy(buffer + old, tmp, nread);
    }
    if (!buffer) { fprintf(stderr, "{\"erro\": \"nenhum dado\"}\n"); return 1; }
    buffer[capacity] = '\0';

    Results res = parse_JSON(buffer);
    free(buffer);
    if (!res.blockchain || !res.componenteCadastro) {
        fprintf(stderr, "{\"erro\": \"parse JSON falhou\"}\n");
        free_results(&res);
        return 1;
    }

    const char* status = verificarBlockchain(res.blockchain, res.componenteCadastro);
    printf("{\"status\": \"%s\"}\n", status);
